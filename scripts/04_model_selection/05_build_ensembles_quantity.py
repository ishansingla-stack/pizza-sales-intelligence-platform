"""
Build Ensemble Configurations for Sales Volume Prediction
...
print("=" * 80)
print("BUILD ENSEMBLE CONFIGURATIONS - SALES VOLUME PREDICTION")
print("=" * 80)
...
results_path = Path(project_root) / "outputs" / "results" / "ml_model_tracking" / "quantity_prediction_runs.csv"
...
def create_model_from_row(row):
    ...
    for param in tuple_params:
        if param in params and isinstance(params[param], str):
            params[param] = eval(params[param])  # Convert '(50,)' to (50,)
...
# Save
output_path = Path(project_root) / "outputs" / "results" / "ml_model_tracking" / "quantity_prediction_runs.csv"
