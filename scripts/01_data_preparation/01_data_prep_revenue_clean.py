"""
Pizza Intelligence - Clean Data Preparation for Revenue Prediction
NO DATA LEAKAGE - Excludes 'Quantity' and 'total_price' from features
Target: total_price (revenue)
Output: data/processed/revenue_target/
"""
...
print("=" * 80)
print("CLEAN DATA PREPARATION - REVENUE PREDICTION (NO LEAKAGE)")
print("=" * 80)
...
output_dir = Path(project_root) / "data" / "processed" / "revenue_target"
...
y_train.to_frame('total_price').to_parquet(output_dir / "y_train.parquet")
y_val.to_frame('total_price').to_parquet(output_dir / "y_val.parquet")
y_test.to_frame('total_price').to_parquet(output_dir / "y_test.parquet")
...
Data Leakage Prevention:
  - EXCLUDED: total_price (the target itself)
  - EXCLUDED: Quantity (can derive revenue from it)
  - INCLUDED: Unit_Price (known pizza attribute)

Next Steps:
  1. Re-run: python scripts/02_sales_regression_training.py
