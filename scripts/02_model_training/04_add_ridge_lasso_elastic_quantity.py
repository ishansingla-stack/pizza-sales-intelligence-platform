"""
Add Ridge, Lasso, ElasticNet Models to Sales Volume Prediction
...
print("=" * 80)
print("ADD RIDGE, LASSO, ELASTICNET - SALES VOLUME PREDICTION")
print("=" * 80)
...
mlflow.set_experiment("/pizza-quantity-prediction")
print(f"[OK] MLflow experiment: /pizza-quantity-prediction")
...
data_dir = Path(project_root) / "data" / "processed" / "quantity_target"

X_train = pd.read_parquet(data_dir / "X_train.parquet")
X_val = pd.read_parquet(data_dir / "X_val.parquet")
X_test = pd.read_parquet(data_dir / "X_test.parquet")
y_train = pd.read_parquet(data_dir / "y_train.parquet")['quantity']
y_val = pd.read_parquet(data_dir / "y_val.parquet")['quantity']
y_test = pd.read_parquet(data_dir / "y_test.parquet")['quantity']
...
results_path = Path(project_root) / "outputs" / "results" / "quantity_volume_results.csv"
...
#%% Build NEW Ensemble with All 10 Models
print("\n" + "=" * 80)
print("STEP 6: BUILD ENSEMBLE FROM TOP 3 MODELS (ALL 10 MODELS)")
print("=" * 80)

# Get top 3 diverse models by validation RMSE
print("\n[*] Top 3 models for ensemble:")
top_3_diverse = []
seen_model_types = set()

for idx, row in all_results_df.nsmallest(len(all_results_df), 'val_rmse').iterrows():
    model_type = row['model_name']
    if model_type not in seen_model_types:
        top_3_diverse.append(row)
        seen_model_types.add(model_type)
        print(f"  {model_type:20s} | Val RMSE: {row['val_rmse']:.4f}")
...
output_path = Path(project_root) / "outputs" / "results" / "quantity_volume_results.csv"
...
#%% Final Summary
print("\n" + "=" * 80)
print("FINAL SUMMARY - SALES VOLUME PREDICTION (UPDATED)")
print("=" * 80)
...
print("\n" + "=" * 80)
print("[COMPLETE] Sales volume prediction training updated!")
