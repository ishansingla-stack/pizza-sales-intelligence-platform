"""
Demand Forecasting - Ensemble Training ONLY
Trains 8 ensemble configurations using the 56 completed base models
"""
...
print("=" * 80)
print("DEMAND FORECASTING - ENSEMBLE TRAINING ONLY")
print("=" * 80)
...
# Load results from completed base training
print("\n[*] Loading completed base model results...")
results_dir = Path(project_root) / "outputs" / "results" / "ml_model_tracking"
results_csv = results_dir / "demand_forecasting_runs.csv"

if not results_csv.exists():
    # Load from MLflow if CSV doesn't exist
    client = mlflow.tracking.MlflowClient()
    exp = client.get_experiment_by_name('/pizza-demand-forecasting')
    runs = client.search_runs(experiment_ids=[exp.experiment_id], max_results=100)

    results_list = []
    for run in runs:
        if run.data.params.get('model_type') != 'Ensemble':
            results_list.append({
    ...
    results_df_temp = pd.read_csv(results_csv)
    # Filter to base models only
    results_df_temp = results_df_temp[results_df_temp['model_name'] != 'Ensemble']

print(f"[OK] Loaded {len(results_df_temp)} base model results")
...
# Model configs (same as original training)
model_configs = {
    "DecisionTree": {
...
# Ensemble configurations - 8 ensembles
print(f"\n{'='*80}")
print("TRAINING 8 ENSEMBLE MODELS")
print(f"{'='*80}")
...
print(f"{'='*80}")
print(f"\n[OK] Trained 8 ensemble models")
print(f"[OK] Total runs in experiment: 56 base + 8 ensemble = 64")
